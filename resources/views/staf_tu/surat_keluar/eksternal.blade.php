...
@section('manajemen-button-tambah')
  <div class="wrapper ml-auto action-bar">
    <a onclick="tambahSuratKeluarEskternal()" style="color:white;" class="btn btn-primary btn-flat" title="Tambah Surat Masuk"><i class="fa fa-plus text-white"></i>Tambah Surat Keluar</a>
  </div>
@endsection
@push('styles')
    <style>
...
@include('staf_tu/surat_keluar.form_eksternal')
    ...
    <div class="row">
        <div class="col-12 table-responsive">
            <table id="table-surat-keluar-eksternal" class="table dt-responsive table-hover table-striped table-bordered nowrap " style="width:100%;">
                <thead>
                    <tr class="tr-header">
                        <td>No</td>
                        <td>Tipe Surat</td>
...
@push('scripts')
    <script>
        $('#table-surat-keluar-eksternal').DataTable({
            responsive: true,
            processing: true,
            serverside: true,
            ajax: "{{ route('staf_tu.surat_keluar_eksternal.api') }}",
            columns: [
        ...
        function tambahSuratKeluarEskternal(){
            save_method = 'add';
        ...
        function editSuratKeluarEksternal(id){
            save_method = 'edit';
            $('input[name=_method]').val('PATCH');
            $.ajax({
                url: "{{ url('staf_tu/surat_keluar_eksternal') }}"+'/'+ id + "/edit",
                type: "GET",
                dataType: "JSON",
                success: function(data){
                    ...
                    // $('#form-teruskan').show();
                    $('#penerima_surat').val(data.penerima_surat);
        ...
        function hapusSuratKeluarEksternal(id){
            ...
            swal({
                ...
                $.ajax({
                    url : "{{ url('staf_tu/surat_keluar_eksternal') }}" + '/' + id,
                    type : "POST",
                    data : {'_method' : 'DELETE', '_token' : csrf_token},
                    success : function(data) {
                        $('#table-surat-keluar-eksternal').dataTable().api().ajax.reload();
                        swal({
                            title: 'Berhasil!',
    ...
    headers: {
        'X-CSSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                ...
                if (!e.isDefaultPrevented()) {
                    var id = $('#id').val();
                    if (save_method == 'add') url = "{{ url('staf_tu/surat_keluar_eksternal') }}";
                    else if(save_method == 'edit')url = "{{ url('staf_tu/surat_keluar_eksternal').'/' }}"+id;
        ...
        $(document).ready(function(){
            ...
            // alert(no_surat);
            url = "{{ url('staf_tu/surat_keluar_eksternal/cari_no_surat') }}";
            $.ajax({
                url :url,
        ...
        //         if (!e.isDefaultPrevented()) {
        //             var id = $('#id').val();
        //             if (save_method == 'teruskan') url = "{{ url('staf_tu/surat_keluar_eksternal').'/' }}"+id;
        //             $.ajax({
        //             url : url,
        //             type : "POST",

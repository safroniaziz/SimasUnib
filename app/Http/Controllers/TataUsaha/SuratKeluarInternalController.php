...
class SuratKeluarInternalController extends Controller
{
    public function index(){
        ...
        return view('staf_tu/surat_keluar.internal',compact('waktu2','id_penerima','id_pengirim','id_penerima_disposisi'));
    }
    ...
    public function dataTable(){
        ...
        $model = DB::table('tb_surat_keluar')
                ...
                ->join('tb_satuan_kerja as id_satuan_kerja_penerima','id_satuan_kerja_penerima.id','tb_surat_keluar.id_satuan_kerja_penerima')
                ->where('tb_surat_keluar.tipe_surat','internal')
                ->where('id_satuan_kerja_pengirim.id',$id_satuan_kerja[0])
                ->select('tb_surat_keluar.id','tipe_surat','id_satuan_kerja_pengirim.nm_satuan_kerja_singkat as satker_pengirim_surat','id_satuan_kerja_penerima.nm_satuan_kerja_singkat as satker_penerima_surat','tb_jenis_surat.jenis_surat','no_surat','perihal','tujuan','lampiran','catatan','sifat_surat','tanggal_surat','tb_surat_keluar.status',DB::raw('@rownum  := @rownum  + 1 AS rownum'))
                ->get();
        return DataTables::of($model)
                ...
                ->addColumn('action', function($model){
                        return
                        '<a onclick="editSuratKeluarInternal('.$model->id.')"  class="btn social-btn btn-primary " style="padding:5px;font-size:10px;"><i class="fa fa-edit"></i></a> '.
                        '<a onclick="hapusSuratKeluarInternal('.$model->id.')"  class="btn social-btn btn-danger " style="padding:5px;font-size:10px;"><i class="fa fa-remove"></i></a> ';
